...
The task detail, date picker, and dragging update passed these checks:

- **540 frontend tests passed.** Coverage includes atomic schedule saves/cancellation, incoming changes, task switching, calendar keyboard and focus behavior, drag ordering, list/tag/board destinations, pointer cancellation, release-click suppression, edge scrolling, and Undo preserving unrelated edits. Existing editor, scheduling, completion, calendar, storage, migration, and Open checkboxes checks also pass.
- Browser mouse checks confirmed reordering, returning to priority order, sidebar list/Inbox moves, tag drops that preserve the list, and Undo. Date checks confirmed multiple work days and a separate deadline saved together, with cancellation preserving existing values.
- iPhone simulator touch checks confirmed the compact detail layout, date sheet, visible selected days, Save/Cancel controls, and cancellation preserving the note and saved schedule.
- The installed Mac app passed mouse reordering, sidebar list/tag drops, tag Undo, and saved-order verification after quit/reopen. Only temporary test tasks were changed; they and their test list were soft-deleted after verification, with backups retained. Existing personal tasks and the chosen iCloud workspace were preserved.
- The production web build, installed Mac app, iPhone/iPad simulator build, and unsigned device Release archive succeeded. All **62 bundled web files** match across those outputs, with no workspace or development directories bundled.
