...
The Open checkboxes update passed these checks:

- **501 frontend tests**, including completed/trashed parent exclusion, immediate count changes, completion/Undo/reopening with unchanged notes, nested checkbox updates, synced changes, and checkbox search. Existing editor, scheduling, completion, calendar, storage, and migration coverage also passes.
- Earlier checks passed **36 native persistence tests** and **24 isolated calendar tests**. Native code is unchanged in this update.
- The Mac app built, installed, and launched with existing tasks and the chosen iCloud workspace preserved. Its Open checkboxes pane showed only active-task steps with matching navigation and footer counts. No personal tasks or calendar records were modified for these checks.
...
- Browser checks confirmed that completing a fictional parent task removed its steps and reduced the checkbox count while the note checkbox remained unchecked; reopening restored its group and count. Search stayed in Open checkboxes, with correct filtered counts and empty-state wording.
- Earlier browser and iPhone simulator checks covered slash insertion, continued typing, math rendering, and popup placement above the software keyboard. Only fictional test notes were edited.
- Earlier live checks covered completion animation and Undo, sound settings, month overflow, and coral calendar badges. Native speaker output has not been listening-tested.
- Earlier iPhone live checks covered calendar permission, event creation/title editing, bidirectional links, and forms above the software keyboard. The fictional event remains in the simulator.
