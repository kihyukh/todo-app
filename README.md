...
- **Predictable note navigation.** Enter and Shift+Enter both start a new block: a paragraph or the next bullet/checkbox; an empty item exits the list. Code and LaTeX source keep literal syntax newlines. Tab / Shift-Tab indents/outdents the current item, and ⌘ Shift Enter toggles its checkbox without moving the cursor. ⌘ Enter continues below a quote, table, code block, or selected image. Click a link to open it immediately, including from elsewhere in Vim Normal mode; opening keeps the editing cursor in place. Option/Alt-click places the caret for editing. On Mac, web links use the system browser and PDF attachments use the system PDF viewer. The browser preview opens web links in the current tab and renders stored PDFs locally with selectable text and page controls. On touch devices, tapping a link offers Open/Edit actions. A short footer hint follows the current block.
...
- **Lists and boards.** Add, rename, or recolor lists. Use a list's **… → Delete list** or right-click it in the sidebar to delete it. The confirmation explains how many active tasks will move to Inbox; completed tasks stay completed and trashed tasks stay in Trash. Notes, files, dates, and tags are preserved. **Undo** restores the list and its tasks without reverting later edits or moves to other lists. Inbox cannot be deleted. Switch list/board layouts. Add, rename, recolor, reorder, or remove board columns. Drag a card between columns or change its status in the detail pane. Removing a column moves its tasks into a remaining column.
...
- The Mac window fills its former title strip, with the visible app title and sidebar branding removed. Window dragging is limited to the window-button reserve, workspace headings, and task-detail header. Blank pane bodies retain normal behavior. The iPhone simulator confirmed the compact search row and drawer dismissal.
- **611 frontend tests passed**, including 11 pane-layout tests and six responsive task-detail integration tests. Responsive coverage includes saved widths, comfortable docking thresholds, retained editor DOM/focus/caret/draft/scroll, date drafts, task switching, dismissal, and phone navigation. All existing editor, window dragging, scheduling, calendar, storage, and migration checks also pass.
- Browser checks confirmed the floating pane at 860px, three columns at 984px and wider, an uninterrupted note edit and Undo across resizing, retained date chooser, and full-screen details with Back navigation at 390px.
- The installed Mac app confirmed floating details, docking when the window was zoomed wider, and returning to floating details with the same selected task when restoring the original size.
