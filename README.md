...
The slash-menu update passed these checks:

- **495 frontend tests**, including slash filtering and insertion, cursor and Vim preservation, undo, image-picker cancellation, synced updates, and the existing editor, scheduling, completion, calendar, storage, and migration coverage.
- Earlier checks passed **36 native persistence tests** and **24 isolated calendar tests**. Native code is unchanged in this update.
- The Mac app built, installed, and launched with existing tasks and the chosen iCloud workspace preserved. This ad hoc development rebuild requests calendar consent again. No personal calendar events were created, edited, or deleted.
...
- Browser checks confirmed `/check`, `/h2`, and `/math`, keyboard and mouse selection, continued typing, and rendered math. The popup remains anchored to the slash while filtering. On the iPhone simulator, a slash entered from the software keyboard opened the popup above it; tapping Checklist kept the keyboard and editing position. Only fictional test notes were edited.
