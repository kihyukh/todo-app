...
- **Optional Vim mode.** Turn on **Settings → Editor → Vim mode** for Normal, Insert, and Visual editing in rich notes. Use `i` / `Esc`, `h j k l`, `w b e`, `0 $`, `gg G`, `dd`, `cw`, `yy p`, and `u` / `Ctrl-R`. Motions enter equation source; leaving restores the previous Normal/Insert mode. Visual selections and operators treat each equation as one unit. The setting is remembered on each device. Equation source fields and the Markdown source panel remain ordinary text inputs.
...
- 102 automated frontend tests cover date independence, task visibility, merge convergence, deletion preservation, concurrent browser saves, rich Markdown/math round trips, nested checkbox updates, directional math navigation, list/quote/table cursor behavior, link clicks, Vim commands, typing batches, immediate task switches, and shutdown flushes.
