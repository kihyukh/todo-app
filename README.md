...
- **Predictable note navigation.** Enter continues bullets and checkboxes; an empty item exits the list. Tab / Shift-Tab indents/outdents the current item, and ⌘ Shift Enter toggles its checkbox without moving the cursor. ⌘ Enter continues below a quote, table, code block, or selected image. Click a link to open it; Option/Alt-click places the caret for editing. On Mac, web links use the system browser and PDF attachments use the system PDF viewer. The browser preview opens web links in the current tab and renders stored PDFs locally with selectable text and page controls. On touch devices, tapping a link offers Open/Edit actions. A short footer hint follows the current block.
