...
## Release identity and remaining checks

| Check                               | Status                                                                                                  |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------- |
| Mac screenshot                      | Capture and upload still required                                                                       |
| Remaining native and provider tests | Core iCloud/relaunch/PDF checks passed; offline, attachment import, and calendar-provider checks remain |
...
Both platform listings' **1.0.0** versions, descriptions, promotional text, keywords, support/marketing URLs, copyright, and **sign-in required: false** are saved and verified. The iOS selected build **1.0.0 (1)**, contact, and review notes were confirmed after reloading. The same authorized private contact and notes were saved for Mac with selected build **1.0.0 (1)**; Save is disabled and there are no validation errors. The metadata file deliberately retains `reviewContact.phone: null` because that value was entered privately in App Store Connect. After the user completed the App Store Connect steps, a live check on **15 September 2026** confirmed App Privacy is **Published**, with **Data Not Collected** and the public privacy-policy URL. Digital Services Act Compliance reports: **“You have completed all regulatory requirements at this time.”** No trader classification or private declaration details are inferred or recorded here.

The user completed membership renewal. The release scripts use Xcode's authenticated account; no membership purchase or private account credential is embedded in repository files. The successful distribution export verifies current signing access. App Store Connect Business also confirms **Free Apps Agreement: Active**. The current Digital Services Act compliance and privacy-publication requirements are complete, as verified above.
...
3. Generate Xcode's privacy report. Current native manifests declare own-app preferences (`CA92.1`); Mac also declares elapsed time between in-app events (`35F9.1`) for window dragging. The published App Privacy response says Data Not Collected, based on the exact archive and publisher practices documented in [privacy-answers.md](../app-store/privacy-answers.md).
