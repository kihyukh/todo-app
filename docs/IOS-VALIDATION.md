...
The remaining release gates are the Mac screenshot and physical-device runtime, iCloud, and calendar-provider checks. Physical-device results are still pending; the earlier simulator, sandbox, signing, and package audits below do not establish those results.

### Physical iPhone preparation

An iPhone 16 Pro Max running iOS **26.6.1** was connected and paired. The user enabled Developer Mode and restarted the phone; device inspection then confirmed Developer Mode enabled and developer services available.

A clean **Release** build, **GreenDay QA 1.0.0 (1)**, was signed with Apple Development under team `3D637V9C9W`, using isolated identifier **`app.daymark.releaseqa.mobile`**. Its development profile includes the connected phone, and strict code-signature verification passed. All **62 web files, 2,572,298 bytes**, match the uploaded release byte for byte, and its privacy manifest also matches. The device installation command returned **success**. The test build is retained at `~/Library/Caches/GreenDay/AppStore/PhysicalDeviceQA/DerivedData/Build/Products/Release-iphoneos/Daymark.app`; neither store archive was rebuilt or replaced.

The dedicated **GreenDay Release Test** folder in iCloud Drive contains seven fictional tasks and a small test PDF. The original cache fixture was preserved, and the user's existing Daymark workspace was not modified. The test apps have not yet connected to this folder; creating a local iCloud folder does not verify delivery to the phone.

With the user's explicit approval, iPhone Mirroring setup proceeded without forwarding phone notifications. Interactive testing is awaiting the user's Mac authentication in Apple's iPhone Mirroring window. Installation alone does not establish launch, touch editing, background persistence, attachment opening, or cross-device sync success.
