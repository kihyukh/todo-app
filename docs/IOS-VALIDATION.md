...
- The final Mac, iPhone/iPad simulator, and unsigned device Release archive builds succeeded. The installed Mac app launched under the GreenDay name with existing tasks and its chosen iCloud workspace preserved. An earlier build of this update loaded configured calendar events after permission approval, using the same native calendar implementation and without any personal calendar writes. The final ad hoc Mac rebuild requires consent again.
