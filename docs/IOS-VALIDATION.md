...
The chosen display name is **GreenDay**, with a green icon and interface accent. The iPhone/iPad scheme is `Daymark` (`app.daymark.mobile`); the separate Mac App Store scheme is `DaymarkMac` (`app.daymark.desktop`). The unsigned verification archives described below use version 1.0.0, build 1, Xcode 26.6, and the platform 26.5 SDKs. Their minimum deployment targets are iOS 16 and macOS 13. They are verification artifacts, not signed store packages.

## Current release status — 15 September 2026

Both App Store Connect platform versions have build **1.0.0 (1)**, complete private review contacts, and review notes saved and verified. Both **1.0.0 (1)** versions were submitted on 15 September 2026 and show **Waiting for Review**: Mac at **22:21 KST**, iOS at **22:23 KST**. Automatic release after approval was verified for both; neither is approved or publicly released yet. After the user completed the App Store Connect steps, live verification confirmed App Privacy is **Published**, with **Data Not Collected** and the public policy URL, and Digital Services Act Compliance reports **“You have completed all regulatory requirements at this time.”** No trader classification or private declaration details are inferred or recorded here.

The Mac screenshot is uploaded and accepted. Offline/reconnection, attachment-import, and calendar-provider checks remain follow-up coverage, rather than identified submission blockers. The physical iPhone pass below verifies a limited two-way iCloud workflow and relaunch persistence; it does not establish every provider or editing behavior.

### Physical iPhone validation

An iPhone 16 Pro Max running iOS **26.6.1** was connected and paired. The user enabled Developer Mode and restarted the phone; developer services were then available. **GreenDay QA 1.0.0 (1)** is a clean Release build signed with Apple Development under team `3D637V9C9W`, with isolated identifier **`app.daymark.releaseqa.mobile`** and a profile including the phone. Strict signature verification passed. Its **62 web files, 2,572,298 bytes**, and privacy manifest match the uploaded release; all 32 file-backed native executable sections also match. Expected differences are the QA name/identifier, development signing/profile, and device-thinned assets. The package remains at `~/Library/Caches/GreenDay/AppStore/PhysicalDeviceQA/DerivedData/Build/Products/Release-iphoneos/Daymark.app`; the store archives were not changed.

After the user authenticated iPhone Mirroring, centering its window resolved the capture cutoff. The following checks used the physical phone through Mirroring and a dedicated **iCloud Drive/GreenDay Release Test** folder. Only fictional fixtures were used; the original cache fixture and the user's existing Daymark workspace were preserved.

| Check                              | Verified result                                                                                                                                                    |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Folder selection                   | The Files picker selected GreenDay Release Test, and all seven fictional tasks loaded, including the test PDF.                                                     |
| PDF opening                        | One tap opened native Quick Look and rendered the test PDF, both initially and again after the fresh app relaunch.                                                 |
| iPhone → Mac checklist             | Checking “Confirm this step” on iPhone appeared checked in the Mac UI and as `checked: true` in its iCloud JSON (`updatedAt: 2026-09-15T12:46:56.591Z`).           |
| Mac → iPhone task                  | The Mac-created “Mac to iPhone sync check” task and its exact note appeared in the physical iPhone UI.                                                             |
| Note editing and background save   | Return followed by individual key input added a `QA` paragraph. It remained after backgrounding and reopening the app.                                             |
| Fresh relaunch and folder bookmark | Only the QA app was swipe-closed in the app switcher, then relaunched from Home. The workspace returned without another picker prompt, and the `QA` note remained. |
| iPhone → Mac note                  | A read-only check of the Mac's iCloud task JSON confirmed the `QA` paragraph (`updatedAt: 2026-09-15T12:54:07.435Z`).                                              |

This establishes narrow two-way task, note, and checklist delivery through iCloud, plus bookmark restoration and note/PDF persistence after relaunch. It does not measure general sync latency or conflict recovery. No offline, real calendar-provider, or image-import test has completed. An App Store-installed build was not tested because this is development-signed QA.

**Date Save: user-confirmed direct check.** Automated Save or subsequent clicks sometimes dismissed the chooser without persisting changes. The user then tested the picker directly in iPhone Mirroring and confirmed that all three work dates were saved. This is a user-reported pass; the subsequent automated capture and Mac-side test record still showed the earlier single date, so this pass does not independently verify cross-device delivery of those dates. No app bug was reproduced. The new `tests/task-schedule-sync.test.tsx` covers unrelated native updates while drafting and between Save pointerdown and click, including a stale cloud echo after saving. All **47 focused regression tests**, TypeScript, and formatting checks passed without changing app code.
...
These audits did not install either distribution package, open the user's workspace, alter its calendar permissions, or upload a build. Signing keys remain in Keychain. Portal verification confirmed the explicit identifier `com.kihyukh.greenday` under the publisher's team. App Store record **6812229239** was created for both platforms with SKU **greenday-2026** and English (U.S.). The available listing name is **GreenDay: Tasks & Notes**; the app itself remains named GreenDay. At that registration stage, both platforms were in Prepare for Submission; their later App Review submissions are recorded below.
...
The Mac **1.0.0 (1)** upload from its audited archive succeeded at **16:07:29 KST**, with command exit status **0**. `build/macos-upload.log` records “Uploaded package is processing,” “Upload succeeded,” and `EXPORT SUCCEEDED`. It used the same export options and output directory `~/Library/Caches/GreenDay/AppStore/macOS-Signed/Upload`. No retry or rebuild was performed. Before the later App Review submissions, App Store Connect confirmed upload status **Complete** and build status **Ready to Submit** for both platforms. Both platform versions now have selected build **1.0.0 (1)**, complete review contacts, and review notes saved. The user-saved iOS contact, build, and notes were verified after reload. The same authorized private contact and notes were then saved for Mac with its selected build; Save is disabled and no validation errors remain. The private phone number is intentionally not stored in repository files.

App Store Connect verification confirmed saved iOS and Mac listing versions **1.0.0**, description, promotional text, keywords, support/marketing URLs, copyright, and no sign-in requirement. Pricing is saved at **US$0.00**, with zero prices verified for all **174 other territories**. Availability is saved for **all 175 countries/regions**, including future territories; the table identifies them as available when the app releases. These settings do not make the app live before approval/release. The Mac screenshot is now accepted; narrow physical-device runtime/iCloud checks passed, with broader provider coverage still unverified; review contact information is saved for both platforms. App Store Connect Business confirms **Free Apps Agreement: Active**. The privacy response was initially saved as a draft; the subsequent live check on 15 September 2026 confirmed **Published**, **Data Not Collected**, and the public policy URL. Digital Services Act Compliance also confirmed all regulatory requirements complete at that time. The trader classification and private declaration details are not recorded here.

## App Review submissions — 15 September 2026

Mac **1.0.0 (1)** was submitted at **22:21 KST** and iOS **1.0.0 (1)** at **22:23 KST**. App Store Connect confirmed **Waiting for Review** for both. Automatic release after approval was verified independently for each platform. The [submission records](APP-STORE.md#testflight-and-public-submission) contain the direct App Store Connect links. This completes submission; it does not claim approval or public release.
...
- Finish the remaining native interaction checks: direct-touch link Open/Edit, tag sheets, image/PDF import, and folder error recovery. Folder selection, relaunch restoration, and one-tap PDF preview before and after relaunch passed on the physical iPhone. The user confirmed date saving directly; independent date-transfer verification is still separate. The required screenshot sets are accepted.
- Both selected builds **1.0.0 (1)**, complete private review contacts, and review notes are saved for App Store record **6812229239**. No further contact, build-selection, privacy-publication, screenshot, or current Digital Services Act compliance action is required. Content Rights was persisted after submission validation identified the missing save; the outer App Information Save confirmed Saved.
- Extend the physical-iPhone checks above: test image/PDF import and offline edits/reconnection, and exercise calendar-provider behavior. Background/relaunch persistence, folder and PDF restoration, and narrow two-way task/note/checklist iCloud delivery have passed; broader provider behavior remains unverified.
- The Mac screenshot is accepted as recorded in [App Store delivery](APP-STORE.md). Both platform listings, publisher/copyright, public support URLs, free prices, and availability in all 175 countries/regions are saved and verified. App Privacy is published and the current Digital Services Act compliance requirements are complete. Future in-app purchases are not configured.

These unverified scenarios are follow-up coverage, not reproduced app defects or mandatory gates for submission. Both versions **1.0.0 (1)** are now submitted and **Waiting for Review**. Automatic release after approval is selected; approval and public availability remain pending.
...
The final iPad UI successfully opened the work-day picker, selected and saved work days, and opened Open checkboxes. Native CUA taps on iPhone header controls remained unreliable; this is an unresolved automation limitation and does not establish physical-touch correctness. The subsequent physical QA and user-confirmed date Save are recorded above; broader direct-touch controls remain follow-up coverage.

The user supplied a native Mac QA screenshot showing fictional Research tasks. Its full 2840 × 1840 image was uniformly scaled and centered on an opaque 2880 × 1800 canvas without cropping or stretching. App Store Connect accepted the image in the Mac slot, with **1 of 10** persisted after saving. The JPEG and preparation/hash manifest are retained under `app-store/screenshots/macos`. No browser screenshot or private workspace was substituted.
