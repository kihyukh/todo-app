...
TickTick's installed Mac app was also inspected at narrow and wide window sizes. Clicking a task in its narrow layout opens a full-height pane against the right edge, with a subtle left shadow and a close button; the exposed list stays clickable. Widening the window docks the selected task into a third column. GreenDay follows this interaction, and also keeps the selected task open when narrowing again. Clicking the exposed workspace dismisses the floating pane; clicking another task switches it directly. Escape dismisses it outside text entry after nested menus have handled the key.

The layout docks when the preferred sidebar width plus a 360px task list and a 400px detail pane fit (984px with the default sidebar). The floating pane uses 360–560px depending on the saved width and remaining list space. Resizing does not remount the editor or overwrite saved pane widths. The navigation divider remains usable in the floating layout; the detail divider returns when docked. At 600px or less, the phone retains its full-screen detail view and Back button.

Opening a closed floating pane slides it in from the right over 220ms with a gentle ease-out. Switching tasks and resizing between layouts do not replay the entrance; an interrupted animation is canceled. Reduced Motion opens the pane immediately. The floating pane's close button sits at the left of the detail header, before completion and dates. The docked three-column layout omits it, and the phone keeps its existing Back button.
...
## Files dropped into notes

Dragging external files over the detail pane shows the insertion caret. Dropping over text inserts plain filename hyperlinks inline; dropping below or between blocks creates paragraphs at the indicated boundary. This applies to dropped images too. Files do not also appear in the attachment list. Typing can continue while bytes are stored, and the anchor follows those edits. Multiple files retain their order, partial failures preserve successful imports, and each drop is undoable. Switching tasks or leaving the rendered editor cancels stale insertion. Internal text/image drags retain their original behavior.

Normal desktop clicks open web links and activate file links; Option/Alt-click keeps direct mouse text editing available. Native PDF/image links open the existing preview, and other files open in their default app. The browser previews images and downloads documents with their original names, avoiding blank embedded PDF viewers. Browser bytes live in a separate IndexedDB file store; native bytes are copied to the workspace Attachments directory before a link is saved. Individual dropped files are limited to 25 MB. Browser JSON exports contain these file references, not the separate binary store.
