...
Developer membership was renewed on 15 September 2026, and both iOS and Mac App Store distribution exports passed their signing audits. The active team is `3D637V9C9W`, with shared release identifier `com.kihyukh.greenday` and App Store record `6812229239` (**GreenDay: Tasks & Notes**; the app's displayed name remains GreenDay). Both **1.0.0 (1)** builds were submitted on 15 September 2026 and show **Waiting for Review**: Mac at **22:21 KST**, iOS at **22:23 KST**. Both listings and free global availability are saved, and the Free Apps Agreement is Active. Both platform versions have their **1.0.0 (1)** builds, complete private review contacts, and review notes saved and verified. The phone number was provided privately and is intentionally omitted from repository files. A live App Store Connect check on 15 September 2026 confirmed App Privacy is Published with Data Not Collected and the public policy URL. Digital Services Act Compliance reports that all regulatory requirements are complete at this time; no trader classification or private declaration details are recorded here. One iPhone screenshot, three iPad screenshots, and one Mac screenshot are uploaded and verified. Content Rights was saved with the No answer and the outer App Information Save confirmed Saved. Both platforms are set to release automatically after approval; neither is approved or publicly released yet. For subsequent builds, configure the active team in **Xcode → Settings → Apple Accounts** and supply these explicit values. Release commands intentionally do not silently reuse the verification bundle/version defaults:
...
The development workflow and workspace names/file formats remain stable. See [App Store delivery](docs/APP-STORE.md) for listing copy, privacy/support pages, publisher/contact inputs, and review steps. Physical iPhone QA passed narrow two-way task/note/checklist iCloud delivery, background/relaunch persistence, and PDF reopening; the user confirmed date saving. Offline/reconnection, attachment-import, provider-specific calendar behavior, and App Store installation remain follow-up coverage, rather than known defects or submission blockers.
...
JSON export includes tasks, notes, projects, tags, column definitions, calendar links, and attachment references. To back up attachment binaries and revision history too, copy the entire Daymark folder. A workspace export is not a backup of the connected calendar accounts. Narrow cross-device iCloud delivery passed on the physical QA iPhone; App Store installation and broader provider scenarios remain unverified.
